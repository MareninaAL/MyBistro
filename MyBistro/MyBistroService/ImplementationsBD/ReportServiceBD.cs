...
namespace MyBistroService.ImplementationsBD
{
    public class ReportServiceBD : IReportService
    {
        ...
        public ReportServiceBD(BistroDbContext context)
        {
        ...
        public List<RefregiratorsLoadViewModel> GetRefregiratorsLoad()
        {
            return context.refrigerators
                            .ToList()
                            .GroupJoin(
                                    context.refrigeratorConstituents
                                                .Include(r => r.Constituent)
                                                .ToList(),
                                    stock => stock,
                                    stockComponent => stockComponent.Refrigerator,
                                    (stock, stockCompList) =>
            new RefregiratorsLoadViewModel
            {
                RefregiratorName = stock.RefrigeratorName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Constituent = stockCompList.Select(r => new Tuple<string, int>(r.Constituent.ConstituentName, r.Count))
            })
                            .ToList();
        }
        ...
        public List<AcquirenteVitaAssassinaModel> GetAcquirenteVitaAssassinas(ReportBindingModel model)
        {
            return context.vitaAssassinas
                            .Include(rec => rec.Аcquirente)
                            .Include(rec => rec.Snack)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new AcquirenteVitaAssassinaModel
                            {
                                AcquirenteName = rec.Аcquirente.АcquirenteFIO,
        ...
        public void SaveAcquirenteVitaAssassinas(ReportBindingModel model)
        {
            ...
            //заполняем таблицу
            var list = GetAcquirenteVitaAssassinas(model);
            var fontForCells = new iTextSharp.text.Font(baseFont, 10);
            for (int i = 0; i < list.Count; i++)
            {
                cell = new PdfPCell(new Phrase(list[i].AcquirenteName, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].DateCreate, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].SnackName, fontForCells));
                table.AddCell(cell);
